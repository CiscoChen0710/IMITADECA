...
class FLAME(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.register_buffer('v_template', to_tensor(to_np(flame_model.v_template), dtype=self.dtype))
                ...
                # Custom v_template from obj
        if hasattr(config, 'custom_template_path'):
            v_custom = []
            with open(config.custom_template_path, 'r') as f:
                for line in f:
                    if line.startswith('v '):
                        parts = line.strip().split()
                        v_custom.append([float(parts[1]), float(parts[2]), float(parts[3])])
            v_custom = torch.tensor(v_custom, dtype=self.dtype)
            self.v_template.data = v_custom.to(self.v_template.device)

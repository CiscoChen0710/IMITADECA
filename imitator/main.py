    ...
    def on_fit_start(self, trainer, pl_module):
        if trainer.global_rank == 0:
            ...
            #                os.path.join(self.cfgdir, "{}-lightning.yaml".format(self.now)))
                    # ✅ 修复关键：确保 self.config 是 OmegaConf 支持的类型
            from omegaconf import DictConfig, ListConfig

            try:
                # project config 强制转换为 DictConfig
                conf_to_save = OmegaConf.create(self.config) if not isinstance(self.config, (DictConfig, ListConfig)) else self.config
                OmegaConf.save(conf_to_save, os.path.join(self.cfgdir, f"{self.now}-project.yaml"))
            except Exception as e:
                print("❌ Failed to save project config:", e)

            try:
                # lightning config 强制转换为 DictConfig
                lightning_conf_to_save = OmegaConf.create({"lightning": self.lightning_config}) if not isinstance(self.lightning_config, (DictConfig, ListConfig)) else self.lightning_config
                print("Lightning config:")
                print(OmegaConf.to_yaml(lightning_conf_to_save))
                OmegaConf.save(lightning_conf_to_save, os.path.join(self.cfgdir, f"{self.now}-lightning.yaml"))
            except Exception as e:
                print("❌ Failed to save lightning config:", e)
...
if __name__ == "__main__":
    ...
    try:
        ...
        merged_config = OmegaConf.merge(*configs, cli)
        config = OmegaConf.create(OmegaConf.to_container(merged_config, resolve=True))
        ...
        base_lr = config.model.params.optim_params.lr

        if not cpu:
            print(lightning_config.trainer.gpus)
            #ngpu = len(lightning_config.trainer.gpus.strip(",").split(','))
            gpus = lightning_config.trainer.gpus
            ngpu = gpus if isinstance(gpus, int) else len(str(gpus).strip().split(","))

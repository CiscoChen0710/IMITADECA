...
if __name__ == "__main__":
    ...
    #解析命令行参数
    #argparse.ArgumentParser()是python内置的一个命令行参数解析器
    #可以通过add_argument()方法添加参数，parse_args()方法解析参数
    parser = get_parser()
    parser = Trainer.add_argparse_args(parser)
    ...
    first_run=True
    # resume training from a checkpoint or a logdir
    # if opt.resume is not None, then resume training from the checkpoint or logdir
    if opt.resume:
    ...
    # if not opt.resume, then create a new logdir
    # if opt.resume is None, then create a new logdir
    else:
        if opt.name:
            name = "_"+opt.name
        elif opt.base:

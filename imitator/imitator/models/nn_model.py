...
class imitator(nn.Module):
    ...
    def forward(self, audio, template, vertice, one_hot, criterion, teacher_forcing=True):
        ...
        for i in range(frame_num):
            if i==0:
                style_emb = obj_embedding.unsqueeze(1)
        ...
        if vertice.dim() == 4:
            vertice = vertice.view(vertice.size(0), vertice.size(1), -1)
    ...
    def style_forward(self, audio, seq_name, template, vertice, one_hot, criterion, teacher_forcing=False):
        ...
        if self.transformer_ff_features[seq_name] is None or train_from_scratch:
            ...
            style_emb = self.obj_vector(one_hot).unsqueeze(1)
    ...
    def predict(self, audio, template, one_hot, test_dataset=None):
        ...
        #vertice_out = self.vertice_map_r.decoder(gen_viseme_feat) + template
        # B, T, V3 = vertice_out.shape
        # V = V3 // 3
        # vertice_out = vertice_out.view(B, T, V, 3)
    # ✅ 显式 expand 模板
        while template.dim() > 2:
            template = template.squeeze(1)  # 去除多余的 batch 维

        # 扩展为 [B, T, V*3]
        template_exp = template.unsqueeze(1).expand(-1, frame_num, -1)
        vertice_out = vertice_out_w_style + template_exp

        B, T, V3 = vertice_out.shape
        V = V3 // 3
        vertice_out = vertice_out.view(B, T, V, 3)

        print("Output shape:", vertice_out.shape)
        pred = vertice_out[0, 0]  # 第1帧
        temp = template[0].reshape(-1, 3)
        print("Prediction (first 5 vertices):", pred[:5])
        print("Template (first 5 vertices):", temp[:5])
        print("Diff (displacement):", pred[:5] - temp[:5])

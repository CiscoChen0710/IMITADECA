...
# 这是一个自定义的数据加载器，用于加载VocaSet数据集
# 该数据集包含了声音和人脸的数据，用于训练声音和人脸的联合表征学习模型
# init data: 实际数据列表，每项包含音频、顶点等
# subjects_dict: 主题字典，包含训练、验证、测试的主题
# data_type: 数据类型，train/val/test
# number_identity_cls: 身份类别数,默认为8
# custom_init_id: 自定义的初始化主题，默认初始身份ID
# one_hot_labels: 身份标签的one-hot编码
# default_index: 默认身份在训练主体列表中的索引
...
# train_subjects、val_subjects、test_subjects：训练/验证/测试主体
# sequence_for_*：可选的自定义序列范围

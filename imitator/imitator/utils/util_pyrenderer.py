...
#         print("✅ Camera added to the scene!")  # 添加这一行确认执行
...
#         camera = pyrender.camera.IntrinsicsCamera(fx, fy, Cx, Cy,
#                                             znear=0.05, zfar=20.0, name=None)
...
#         camera_translation[:3, 3] = np.array([0, 0, 20])  # 往后移500个单位
#         camera_pose = camera_rotation @ camera_translation #先旋转再平移
#         #camera_pose = camera_translation
#         self.scene.add(camera, pose=camera_pose)

#     # def add_camera(self, intrinsic):
#     #     (fx, fy, Cx, Cy) = intrinsic
#     #     fy = abs(fy)  # 保证 fy 是正的（有时候导入数据可能是负的）
        
#     #     camera = pyrender.IntrinsicsCamera(fx=fx, fy=fy, cx=Cx, cy=Cy, znear=0.05, zfar=10.0)

#     #     camera_pose = np.eye(4)
#     #     camera_pose[:3, 3] = np.array([0, 0, 1.0])

#     #     self.camera_node = self.scene.add(camera, pose=camera_pose)
#     #     print("✅ Camera added to the scene! Node:", self.camera_node)
...
class Facerender:
    ...
    def reset_scene(self):
        self.scene.clear()
        self.add_camera(self.intrinsic)
        light = pyrender.DirectionalLight(color=[1, 1, 1], intensity=2e3)
        self.scene.add(light, pose=np.eye(4))
    ...
    def add_camera(self, intrinsic):
        ...
        # 不颠倒角度，仅稍微朝前看
        camera_pose = np.eye(4)
        camera_pose[:3, :3] = Rotation.from_euler('x', 0, degrees=True).as_matrix()

        # 比原来的 20 近一点，例如 2.0 单位距离
        camera_pose[:3, 3] = np.array([0, 0, 2.0])

        self.scene.add(camera, pose=camera_pose)

    def add_face(self, vertices, faces, pose=np.eye(4)):
        # 安全地移除旧 mesh_node
        if self.mesh_node is not None and self.mesh_node in self.scene.get_nodes():
            self.scene.remove_node(self.mesh_node)

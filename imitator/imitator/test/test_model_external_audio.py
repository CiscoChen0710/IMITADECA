...
class test_on_audio():
    ...
    def __init__(self):
        ...
        if opt.template_pkl is not None:
            template_file = opt.template_pkl
        elif os.getenv("VOCASET_PATH"):
            template_file = os.path.join(os.getenv("VOCASET_PATH"), "templates.pkl")
        else:
            template_file = os.path.join(os.getenv("HOME"), "projects/dataset/voca_face_former", "templates.pkl")
    ...
    def load_model_from_checkpoint(self, model_ckpt):
        ...
        # print("Logdir", logdir)
        import os
        logdir = os.path.dirname(os.path.dirname(model_ckpt))  # logs/tb/version_3
        print("Using corrected logdir:", logdir)
    ...
    def run_on_wav_file(self, template, model,
        ...
        for condition in condition_for_tesing:
            ...
            #如果要加载预训练model的8indentity，取消这部分注释并将现有的condition_tensor，prediction注释掉
            ...
            prediction = model.nn_model.predict(sampled_processed_audio, template, condition_tensor)
    ...
    def run_test_on_the_audio_path(self, exp_cfg, model_folder, model_ckpt):
        ...
        for subj_name in identity_for_testing:
            if opt.template_obj is not None:
                import trimesh
                print(f"Using template from OBJ file: {opt.template_obj}")
                mesh = trimesh.load(opt.template_obj, process=False)
                template = mesh.vertices.astype(np.float32)
            else:
                print(f"Using template from templates.pkl: {subj_name}")
                template = self.templates[subj_name]
...
def get_parser(parser):
    ...
    parser.add_argument('--template_obj', type=str, default=None,
                    help="Path to custom .obj template (overrides templates.pkl)")
    parser.add_argument('--template_pkl', type=str, default=None, help="Path to templates.pkl (e.g., templates_biden.pkl)")
    return parser

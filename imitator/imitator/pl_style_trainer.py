...
class Full_training(Imitator):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if train_subject in self.nn_model.train_subjects:
            ...
            # reshape ground truth to match pred shape
            if vertice.dim() == 4:
    ...
    def compute_auxillary_losses(self, predicted_mesh, gt_mesh):
        ...
        # return loss_dict
            # 🔧 修复维度不一致问题
        if gt_mesh.dim() == 4:
            gt_mesh = gt_mesh.view(gt_mesh.size(0), gt_mesh.size(1), -1)
        if predicted_mesh.dim() == 4:
            predicted_mesh = predicted_mesh.view(predicted_mesh.size(0), predicted_mesh.size(1), -1)

        velocity_loss, velocity_loss_weighted = self.custom_loss.velocity_loss(predicted_mesh, gt_mesh)
        aux_loss_sum = velocity_loss_weighted
        loss_dict = {
            "aux_losses": aux_loss_sum,
            "velocity_loss": velocity_loss,
